...
import { Keypair, StrKey, TransactionBuilder, BASE_FEE, Networks, Operation, Account, } from "@stellar/stellar-sdk";
import { env } from "../config/env.js";
import { usersService } from "../modules/users/users.service.js";
import { HttpError } from "../utils/http-error.js";
const nonceForAccount = (account) => {
...
export const stellarAuthService = {
    createChallenge(account) {
        ...
        if (!env.STELLAR_SEP10_SECRET) {
            throw new HttpError(501, "SEP-10 signing key is not configured on the server");
        }
    ...
    verifyChallenge(transaction) {
        const parsed = verifyChallengeSchema.parse({ transaction });
        if (!env.STELLAR_SEP10_SECRET) {
            throw new HttpError(501, "SEP-10 signing key is not configured on the server");
        }
        const signer = Keypair.fromSecret(env.STELLAR_SEP10_SECRET);
        const tx = TransactionBuilder.fromXDR(parsed.transaction, env.STELLAR_NETWORK_PASSPHRASE);

...
uploadsRouter.post("/sessions", requireSession, asyncHandler(async (request, response) => {
    const ownsTrack = tracksService.userOwnsTrack(request.session.walletAddress, request.body.trackId);
    if (!ownsTrack) {
...
uploadsRouter.put("/content/*", asyncHandler(async (request, response) => {
    if (!request.body) {
        throw new HttpError(400, "Upload body is required");
    }
    const storageKey = decodeURIComponent(String(request.params[0]));
    await storageService.saveLocalObject(storageKey, Readable.from(request.body));
    response.status(204).send();
}));
uploadsRouter.get("/content/*", asyncHandler(async (request, response) => {
    const storageKey = decodeURIComponent(String(request.params[0]));
...
uploadsRouter.post("/sessions/:uploadSessionId/complete", requireSession, asyncHandler(async (request, response) => {
    ...
    if (!tracksService.userOwnsTrack(request.session.walletAddress, session.trackId)) {
        throw new HttpError(404, "Track not found");
    }

...
export const playbackService = {
    ...
    getManifest(sessionId, token) {
        ...
        const baseUrl = track.streamMediaUrl ??
        ...
        const segmentUrl = `${baseUrl}${baseUrl.includes("?") ? "&" : "?"}playbackSession=${encodeURIComponent(session.id)}`;
        return [
            "#EXTM3U",
            "#EXT-X-VERSION:3",
    ...
    getMediaRedirect(sessionId, token) {
        const session = this.getSession(sessionId, token);
        const track = tracksService.getTrack(session.trackId);
        if (!track?.playbackReady) {
            throw new HttpError(404, "Track media is not available");

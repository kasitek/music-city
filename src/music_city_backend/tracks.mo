...
module {
  public func create(
    ...
    if (!isArtist) { return #err("Only artists can create tracks") };
    let track : T.Track = {
  ...
  public func incrementPlay(tracks : [T.Track], trackId : Nat) : Result.Result<([T.Track], T.Track), Text> {
    let idxOpt = Array.indexOf<T.Track>({ id = trackId; title = ""; artist = Principal.fromText("aaaaa-aa"); duration = ""; genre = ""; coverImage = ""; audioUrl = ""; plays = 0; likes = 0; price = 0; releaseDate = ""; description = "" }, tracks, func (a, b) { a.id == b.id });
    switch (idxOpt) {
      case null { #err("Track not found") };
      case (?idx) {
        let t = tracks[idx];
        let updated = { t with plays = t.plays + 1 };
        let t' = Array.tabulate<T.Track>(tracks.size(), func (i) { if (i == idx) { updated } else { tracks[i] } });
        #ok((t', updated))
      }
    }
  };
